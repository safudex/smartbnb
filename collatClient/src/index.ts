...
const {
  NEO_PRIVATE_KEY, CONTRACT_SCRIPTHASH, REDIS_URL, BNC_PRIVATE_KEY,
} = process.env;
...
const bncApi = new BncApi(BNC_PRIVATE_KEY, NETWORK);
const collatId = neoApi.address + bncApi.address;
neoApi.subscribe(neoEventHandler(neoApi, REDIS_URL, collatId, bncApi));
