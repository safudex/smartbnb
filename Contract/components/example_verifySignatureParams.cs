...
namespace Neo.SmartContract
{
    public class HelloWorld : Framework.SmartContract
    {
        ...
        private static bool ChallengeInitialChecks()
        {
            byte[] signature = getStateFromStorage("signature");
            BigInteger R0_xSigHigh = getStateFromStorage("R0_xSigHigh");
            BigInteger R1_ySigHigh = getStateFromStorage("R1_ySigHigh");
            byte[] signBytes = getStateFromStorage("signBytes");
            byte[] blockHash = getStateFromStorage("blockHash");
        ...
        private static bool ChallengeCheckBytes()
        {
            ulong[] pre = getStateFromStorage("pre");
            byte[] signBytes = getStateFromStorage("signBytes");
            
            return checkBytes(pre, signBytes, 1, (int)pre[pre.Length-1]);
        }
        
        private static bool ChallengeSha512()
        {
            ulong[] pre = getStateFromStorage("pre");
            ulong[] hash = getStateFromStorage("hash");
        ...
        private static bool ChallengeSha512ModQ()
        {
            ...
            BigInteger mod = getStateFromStorage("mod");
        ...
        private static bool ChallengePointEqual()
        {
            byte[] sB = getStateFromStorage("sB");
            byte[] hA = getStateFromStorage("hA");
            BigInteger R0_xSigHigh = getStateFromStorage("R0_xSigHigh");
            BigInteger R1_ySigHigh = getStateFromStorage("R1_ySigHigh");
